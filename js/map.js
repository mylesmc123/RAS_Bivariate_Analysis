...
var map = new maplibregl.Map({
  ...
  center: [-94.5, 29.7], // starting position [lng, lat]
...
map.on('style.load', function () {
  ...
  map.addLayer({
    ...
    'source': {
      type: "geojson",
      data: "../data/GLO RAS Domain.geojson"
    },
    // 'layout': {
    //   'visibility': 'none'
    // },
  ...
  map.on('click', 'timeseries', function (e) {
    ...
    var point_id = e.features[0].properties.id;
    ...
    fetch(`../output/point_jsons/${point_id}.json`)
    .then(response => response.json())
    .then(data => {
      // console.log(data)
